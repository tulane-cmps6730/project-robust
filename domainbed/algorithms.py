...
class PrependPrompt():
    ...
    #     def _add_domain_prompt(model, x):
    #         act = x[0]
    #         x_new = torch.cat([act, domain_tokens.repeat(act.shape[0], 1, 1)], dim=1)
    #         return (x_new, )
    #     return _add_domain_prompt
...
class XXX(ERM):
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super().__init__(input_shape, num_classes, num_domains, hparams)
        self.num_domains = 1    # only target domain
        self.hidden_dim = self.featurizer.network.hidden_dim
        self.prompt_dim = self.hparams['prompt_dim']
        self.mlp_dim = 3072
        assert self.hparams['vit_base_16'] == True
...
import ot
class PromptTTA(Algorithm):
    """
    Empirical Risk Minimization (ERM)
    """     
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(PromptTTA, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.global_iter = 0
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.classifier = networks.Classifier(
            self.featurizer.n_outputs,
        ...
        self.network = nn.Sequential(self.featurizer, self.classifier)
        self.label_distance = hparams['label_distance'] if 'label_distance' in hparams else 1e8
        
        # ckpt = torch.load('/media/zybeich/prompt_tta/results/erm_cs/model.pkl')
        # msg = self.network.load_state_dict(ckpt['model_dict'])
        # assert msg == None
        ...
        # prompt tokens
        self.prompt_tokens = nn.Parameter(
            torch.empty(hparams['prompt_dim'], self.featurizer.network.hidden_dim).normal_(std=0.02)
        )
    ...
    @torch.no_grad()
    def forward_prompt(self, all_x_trg):
    ...
    @torch.no_grad()
    def forward_prompt_raw(self, all_x_trg):
        domain_prompts = self.prompt_tokens
        with PrependPrompt(self.featurizer, domain_prompts.repeat(all_x_trg.shape[0], 1, 1)):
            all_z_trg = self.featurizer(all_x_trg)
        return all_z_trg
    
    def forward_prompt_raw_(self, all_x_trg):
        domain_prompts = self.prompt_tokens
        with PrependPrompt(self.featurizer, domain_prompts.repeat(all_x_trg.shape[0], 1, 1)):
            all_z_trg = self.featurizer(all_x_trg)
        return all_z_trg
    
    def update(self, minibatches, src=None):
        '''
        minibatches: SRC labeled data
        unlabeled: TGT unlabeled/labeled data
        '''
        if src is not None:
            src_features, src_labels = src
        self.prompt_opt.zero_grad()
        ...
        # 
        self.network.eval()
        pseudo_labels = self.predict_pseudo(all_x)
        # all_logit_trg = self.forward_prompt(all_x)
        # loss = F.cross_entropy(all_logit_trg, all_y)
        all_z_trg = self.forward_prompt_raw_(all_x)
        # optimal_couppling, cost_matrix = self.compute_labeled_wd(all_z_trg, all_y, src_features, src_labels)
        optimal_couppling, cost_matrix = self.compute_labeled_wd(all_z_trg, pseudo_labels, src_features, src_labels, label_distance=self.label_distance)
    ...
    def compute_labeled_wd(self, trg_features, trg_labels, src_features, src_labels,  normalize=True, label_distance=1e8):
        device = trg_features.device
        ...
        # curr_wd = (optimal_couppling * cost_matrix).sum()
        return  optimal_couppling, cost_matrix
    
    def predict_pseudo(self, x, domain=None):
        logits = self.forward_prompt(x)
        _, predicted = torch.max(logits, 1)
        return predicted
    
    
class PromptWDTune(Algorithm):
    """
    Empirical Risk Minimization (ERM)
    """     
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(PromptWDTune, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.global_iter = 0
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.classifier = networks.Classifier(
            self.featurizer.n_outputs,
    ...
    def update(self, minibatches, src=None):
        ...
        # loss = F.cross_entropy(all_logit_trg, all_y)
        all_z_trg = self.forward_prompt_raw_(all_x)
        optimal_couppling, cost_matrix = self.compute_labeled_wd(all_z_trg, all_y, src_features, src_labels, label_distance=self.label_distance)
        loss = torch.sqrt((optimal_couppling * cost_matrix).sum())
...
class PromptXENTTune(Algorithm):
    """
    Empirical Risk Minimization (ERM)
    """     
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(PromptXENTTune, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.global_iter = 0
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.classifier = networks.Classifier(
            self.featurizer.n_outputs,
    ...
    def forward_prompt_(self, all_x_trg):
        domain_prompts = self.prompt_tokens
        with PrependPrompt(self.featurizer, domain_prompts.repeat(all_x_trg.shape[0], 1, 1)):
            all_logit_trg = self.network(all_x_trg)
        return all_logit_trg
    ...
    def update(self, minibatches, src=None):
        ...
        all_logit_trg = self.forward_prompt_(all_x)
        loss = F.cross_entropy(all_logit_trg, all_y)
...
class PromptTTA_RN50(Algorithm):
    """
    Empirical Risk Minimization (ERM)
    """     
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(PromptTTA_RN50, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.global_iter = 0
        assert self.hparams['vit_base_16'] == False
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.classifier = networks.Classifier(
            self.featurizer.n_outputs,
            num_classes,
        ...
        # self.hidden_dim = self.featurizer.network.hidden_dim
        self.out_dim = 2048
        self.num_tokens = self.hparams['prompt_dim']    # default is 5
        # self.mlp_dim = 3072
        ...
        self.prompt_opt = torch.optim.SGD(
            [self.prompt_embeddings_tb, self.prompt_embeddings_lr],
            lr=self.hparams["lr_prompt"],
            weight_decay=1e-3
        )
    ...
    @torch.no_grad()
    def forward_prompt(self, all_x_trg):
        x = self.incorporate_prompt(all_x_trg)
        all_logit_trg = self.network(x)
        return all_logit_trg
...
class PromptTune_RN50(Algorithm):
    """
    Empirical Risk Minimization (ERM)
    """     
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(PromptTune_RN50, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.global_iter = 0
        assert self.hparams['vit_base_16'] == False
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.classifier = networks.Classifier(
    ...
    def update(self, minibatches, src=None):
        ...
        all_z_trg = self.forward_prompt_raw_(all_x)
        optimal_couppling, cost_matrix = self.compute_labeled_wd(all_z_trg, all_y, src_features, src_labels, label_distance=self.label_distance)
        # optimal_couppling, cost_matrix = self.compute_labeled_wd(all_z_trg, pseudo_labels, src_features, src_labels, label_distance=1e4)
        loss = torch.sqrt((optimal_couppling * cost_matrix).sum())
...
from domainbed.adapt_algorithms import softmax_entropy

class PromptEnt(Algorithm):
    """
    Empirical Risk Minimization (ERM)
    """     
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(PromptEnt, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.global_iter = 0
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.classifier = networks.Classifier(
            self.featurizer.n_outputs,
    ...
    def update(self, minibatches, src=None):
        ...
        # optimal_couppling, cost_matrix = self.compute_labeled_wd(all_z_trg, pseudo_labels, src_features, src_labels, label_distance=self.label_distance)
        # loss = torch.sqrt((optimal_couppling * cost_matrix).sum())
        loss = softmax_entropy(all_logit_trg).mean(0)
...
class PromptEntTTA(Algorithm):
    """
    Empirical Risk Minimization (ERM)
    """     
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(PromptEntTTA, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.global_iter = 0
        self.featurizer = networks.Featurizer(input_shape, self.hparams)
        self.classifier = networks.Classifier(
            self.featurizer.n_outputs,
        ...
        self.network = nn.Sequential(self.featurizer, self.classifier)
        self.label_distance = hparams['label_distance'] if 'label_distance' in hparams else 1e8
        self.ent_weight = hparams['ent_weight'] if 'ent_weight' in hparams else 1e3
    ...
    def forward_prompt_raw_(self, all_x_trg):
        ...
        with PrependPrompt(self.featurizer, domain_prompts.repeat(all_x_trg.shape[0], 1, 1)):
            all_z_trg = self.featurizer(all_x_trg)
            all_logit_trg = self.classifier(all_z_trg)
        return all_z_trg, all_logit_trg
    ...
    def update(self, minibatches, src=None):
        ...
        # all_logit_trg = self.forward_prompt(all_x)
        # loss = F.cross_entropy(all_logit_trg, all_y)
        all_z_trg, all_logit_trg = self.forward_prompt_raw_(all_x)
        # optimal_couppling, cost_matrix = self.compute_labeled_wd(all_z_trg, all_y, src_features, src_labels)
        optimal_couppling, cost_matrix = self.compute_labeled_wd(all_z_trg, pseudo_labels, src_features, src_labels, label_distance=self.label_distance)
        wd_loss = torch.sqrt((optimal_couppling * cost_matrix).sum())
        ent_loss = softmax_entropy(all_logit_trg).mean(0)
        loss = wd_loss + self.ent_weight * ent_loss
