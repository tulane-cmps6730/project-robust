...
if __name__ == "__main__":
    ...
    parser.add_argument('--src_envs', type=int, nargs='+', default=[-1])
    parser.add_argument('--trg_envs', type=int, nargs='+', default=[-1])
    ...
    if args.dataset == 'PACS':
        envs = [0, 1, 2, 3]
        if args.src_envs == [-1]:
            args.src_envs = [env for env in envs if env not in args.trg_envs]
        if args.trg_envs == [-1]:
            args.trg_envs = [env for env in envs if env not in args.src_envs]
            
    print(f'src_envs: {args.src_envs}, trg_envs: {args.trg_envs}')
    ...
    if args.dataset in vars(datasets):
        dataset = vars(datasets)[args.dataset](args.data_dir,
            args.trg_envs + args.src_envs, hparams)
    else:
    ...
    tta = algorithms.PromptTune_RN50(dataset.input_shape, dataset.num_classes,
                               1, hparams)
    ...
    # domain mapping
    cnt = 0
    domain_mapping = {x: None for x in args.trg_envs}
    # for i in range(len(in_splits)):
    #     if i not in args.test_envs:
    ...
    for step in range(start_step, n_steps):
        ...
        if (step % checkpoint_freq == 0) or (step == n_steps - 1):
            ...
            # print(tta.prompt_embeddings_lr.shape, tta.prompt_embeddings_tb.shape)
            # print(tta.prompt_embeddings_lr.sum(), tta.prompt_embeddings_tb.sum())
            

    save_checkpoint(f'{args.trg_envs[0]}_prompt.pkl')
