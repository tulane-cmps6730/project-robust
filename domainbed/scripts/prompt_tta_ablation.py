...
if __name__ == "__main__":
    ...
    for i, (env, env_weights) in enumerate(out_splits):   
        if i == args.test_envs[0]:
            ...
            # print(f'TRG env: {i}', end='\t')
            break
    print(f'num of trg data: {len(env_list)}')      
    print(f'SRC: {args.train_envs}, TRG: {args.test_envs}')
    
    # print("random sampling")
    # idx = np.random.choice(src_features.shape[0], len(env_list), replace=False)
    ...
    sampled_indices = {class_label: None for class_label in range(dataset.num_classes)}

    for class_label in range(7):
        class_indices = np.where(labels == class_label)[0]
        
        sampled_indices_for_class = np.random.choice(class_indices, size=n_samples, replace=False)
        
        sampled_indices[class_label] = sampled_indices_for_class
        
    sampled_indices = np.concatenate([sampled_indices[class_label] for class_label in sampled_indices])
    src_features = src_features[sampled_indices]
    src_labels = src_labels[sampled_indices]
    ...
    checkpoint_freq = len(env_list) // hparams['batch_size']
    n_steps = (checkpoint_freq + 1) * 20 #min((checkpoint_freq + 1) * 5, 31)
