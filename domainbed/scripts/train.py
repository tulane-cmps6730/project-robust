...
if __name__ == "__main__":
    ...
    in_splits_num = [len(env) for env, _ in in_splits]
    out_splits_num = [len(env) for env, _ in out_splits]
    ...
    for step in range(start_step, n_steps):
        ...
        if (step % checkpoint_freq == 0) or (step == n_steps - 1):
            ...
            for env in args.train_envs:
                total += out_splits_num[env]
                correct += results['env{}_out_acc'.format(env)] * out_splits_num[env]
                
            if best_acc < correct / total:
                best_acc = correct / total
                best_step = step
                save_checkpoint('best_model.pkl')
                print(f"New best!! acc: {best_acc:.3f}, step: {best_step}")
