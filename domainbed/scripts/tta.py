...
def accuracy_ent(network, loader, weights, device, adapt=False):
    ...
    return correct / total * 100, ent / total
...
if __name__ == "__main__":
    ...
    args.test_envs = args_in.test_envs if args_in.test_envs != [-1] else args.test_envs
    ...
    args.output_dir = os.path.join(args_in.output_dir, alg_name)
    ...
    assert os.path.exists(os.path.join(args.input_dir, 'best_model.pkl'))  # IID_best is produced by train.py
    ...
    # load trained model
    ckpt = torch.load(os.path.join(args.input_dir, 'best_model.pkl'))
